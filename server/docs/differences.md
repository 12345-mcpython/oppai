...
| **「功能开启」弹窗** | **不弹**（登录块 `moduleOpenMark` 把 32 个 mark 全标成已弹过） | 原版每个系统第一次开启弹一次；私服建号就 30 级 + 全解锁，一进游戏会**连弹 32 个**。纯服务端开关，客户端一行都不用动 | `store.MODULE_OPEN_POPUP_SKIP = False` 即还原（老存档里被客户端回写的 mark 仍然生效） |
