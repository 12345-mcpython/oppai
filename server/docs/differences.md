...
| A10 | **自己编 `arm64-v8a` 引擎**（原版只有 armeabi/x86） | 原版包的 `lib/` 只有 `armeabi` + `x86`；cocos 官方依赖包在 3.6 那个年代也只有这三套 ABI。真机（一加 PLZ110，`abilist=arm64-v8a`）本来靠厂商 32 位兼容层跑 armeabi-v7a，但那是**兼容层**：性能打折、64 位系统上还多一层转换。2026-09-20 把 arm64 依赖自己凑齐（依赖仓库里的 arm64 预编译 + 自建 chipmunk **6.2.1**、自建 libwebsockets **1.23**、SM 的 64 位 `js-config`、按 ABI 分的 curl 头），编出 64 位 `libcocos2djs.so`（21.6 MB） | 实测真机 `primaryCpuAbi=arm64-v8a`、冷启动正常、登录 + 主界面轮询正常（**原生 64 位，不再走兼容层**）。一条命令可复现：`python script\build_arm64_deps.py` + `.\build.ps1 -Engine -Abi arm64-v8a -PackAbis arm64-v8a`；踩的两个坑（chipmunk 版本、libwebsockets 结构体布局导致 `setgid(0)` 被 seccomp `SIGSYS`）记在 [`build.md`](build.md) 的「ABI」节 |
