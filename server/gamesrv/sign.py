...
# 7 天奖励（**自己定的**，见模块注释）。每天**恰好一件** —— 客户端
# `SignRewardItem._init` 是按件数分支的：`length === 1` 用该道具自己的图标，
# `length > 1` 一律用 `res/signcommonicon` 通用图标（7 个格子长得一模一样，
# 看不出给什么），`length === 0` 直接 `cc.warn` + `addChild(undefined)`。
# 每天一组 `(REWARD_TYPE, key, count)`，和 `table_level_reward` 那套奖励串同格式：
# type 2 = 道具。
SIGN_REWARDS = (
    (("2", "100001", 200),),        # 金条
    (("2", "100003", 30),),         # 行动力
    ...
    (("2", "100002", 5000),),       # 萌钞
    (("2", "200101", 1),),          # 小颗糖果（材料）
    (("2", "100201", 1),),          # BP（好友 BOSS 点）
    (("2", "100004", 50),),         # 第 7 天：萌能水晶 ×50（一周的大奖）
)
