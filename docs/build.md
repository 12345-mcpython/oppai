...
- [第 ③ 步 `build_apk.py` 做什么](#第-③-步-build_apkpy-做什么)
- [ABI：打包哪几个 `.so`，以及能编哪几个](#abi打包哪几个-so以及能编哪几个)
- [URL 怎么进客户端：默认**运行时改写**，老路子才是「等长替换」](#url-怎么进客户端默认运行时改写老路子才是等长替换)
- [幂等性](#幂等性)
- [⚠️ 脚本顺序：`patch_smali.py` 必须排在 `strip.py` **之后**](#-脚本顺序patch_smalipy-必须排在-strippy-之后)
- [⚠️ `.ps1` 必须带 UTF-8 BOM（不只是 `build.ps1`）](#-ps1-必须带-utf-8-bom不只是-buildps1)
- [一键构建（`build.ps1`）为什么要跑 `patch_smali.py`](#一键构建buildps1为什么要跑-patch_smalipy)
- [项目重排留下的两个脚本 bug（已修）](#项目重排留下的两个脚本-bug已修)
- [路径配置](#路径配置)
- [完整重建命令](#完整重建命令)
- [产物](#产物)
- [解包目录里哪些可以删](#解包目录里哪些可以删)
- [实测数据](#实测数据)
...
### 包里现有的四份（`game\lib\<abi>\libcocos2djs.so`）
...
* 要的 ABI 到处都没有时直接报错，并提示怎么编；`arm64-v8a` 会额外提示**先备依赖**
  （`python script\build_arm64_deps.py`，见 [`../engine/ARM64.md`](../engine/ARM64.md)）。

实测体积：三份（armeabi + v7a + x86）全带 **558.2 MB**，四份 **565.7 MB** → 只带 arm 两份 **549.9 MB**（省 8.3 MB = x86 那份压缩后的大小）；
...
`libcocos2djs.so` 是 NDK r10e + cocos2d-x 3.6 + SpiderMonkey 33.1.1 编出来的，依赖一堆预编译库。
cocos 官方依赖包在 3.6 年代**只有 armeabi / armeabi-v7a / x86 三套**：
...
| **`arm64-v8a`** | **自己凑的依赖**：chipmunk 6.2.1 / libwebsockets 1.23 自己编 + 按 ABI 分的头文件 |

> arm64 的依赖怎么凑、四个坑（chipmunk 版本 / libwebsockets 结构体布局 /
> **jpeg 的 `boolean` 宽度导致所有 `.jpg` 静默变黑** / toolchain 与 `APP_PLATFORM`）、
> 以及编完怎么验证，都在 **[`../engine/ARM64.md`](../engine/ARM64.md)**。
> 一条命令备依赖：`python script\build_arm64_deps.py`（`--check` 只报告）。

实测（一加 PLZ110 / Android 16）：`primaryCpuAbi=arm64-v8a`、冷启动正常、能登录、
主界面轮询（`boss.getbosslist` / `sync.syncupclient`）正常、无 JS 报错 ——
**原生 64 位跑，不再走厂商的 32 位兼容层**。
