...
| `route_gap.py` | 把客户端里能当 route 的字符串全抽出来，和服务端已实现的对一遍（`--static` 静态盘点：候选 161 / 服务端已实现 117 / 缺 52）。⚠️ 静态扫是**按字节正则**捞的，有假阳性：`cp.bb` 是 chipmunk 的 bounding box API（`assets/script/chipmunk/jsb_chipmunk.jsc`），不是 route —— 判缺口时顺手 `jsc_find.py <名字>` 或 `out/` 里那种字节查证一下（真实缺口是 51 条） |
